...
def generate_icons(site_meta):
    ...
    for icon_size in tqdm(site_meta['icon_sizes']):
        ...
        image_file_path_png = 'output/{name}-{size}x{size2}.png'.format(
            name="icon", size=icon_size, size2=icon_size)
...
def generate_responsive_images_helper(from_path,
                                      to_path,
                                      images,
                                      image_sizes,
                                      my_que=None):
    ...
    if len(glob.glob(to_path + image_name + "*")) != len(image_sizes) * 2:
        ...
        for image_size in image_sizes:
            ...
            image_file_path_webp = to_path + '{name}-{size}.webp'.format(
                name=image_name, size=image_size)
            image_file_path_jpg = to_path + '{name}-{size}.jpg'.format(
                name=image_name, size=image_size)
            new_image.save(image_file_path_webp, quality=50, method=0)
            new_image.save(image_file_path_jpg, quality=50, optimize=True)
    else:
        print(
            "Image with name " + image_name +
            " generated before, skipping. To regenerate please execute clean.py and repeat."
        )
    if my_que:
        my_que.put("Done")
...
def generate_responsive_images(from_path, to_path, image_sizes):
    ...
    for images in tqdm(os.listdir(from_path)):
        if thread_number == 1:
            generate_responsive_images_helper(from_path, to_path, images,
                                              image_sizes, my_que)
        else:
            start_new_thread(generate_responsive_images_helper, (
                from_path,
                to_path,
                images,
                image_sizes,
